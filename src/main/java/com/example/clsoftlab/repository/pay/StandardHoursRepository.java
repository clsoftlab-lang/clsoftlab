package com.example.clsoftlab.repository.pay;

import java.util.List;

import org.springframework.data.jpa.repository.JpaRepository;
import org.springframework.data.jpa.repository.JpaSpecificationExecutor;
import org.springframework.data.jpa.repository.Query;
import org.springframework.stereotype.Repository;

import com.example.clsoftlab.dto.pay.StandardHoursSearchDto;
import com.example.clsoftlab.entity.StandardHours;

@Repository
public interface StandardHoursRepository extends JpaRepository<StandardHours, Long>, JpaSpecificationExecutor<StandardHours> {

	// 중복 검사
	public boolean existsByCalYmAndJobGroup (String calYm, String jobGroup);
	
	// jobGroup 리스트 조회
	@Query ("SELECT DISTINCT s.jobGroup "
			+ "FROM StandardHours s "
			+ "ORDER BY s.jobGroup ")
	public List<StandardHoursSearchDto> getJobGroupList ();
}
