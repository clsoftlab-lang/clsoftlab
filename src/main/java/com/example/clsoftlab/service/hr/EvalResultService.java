...
import com.example.clsoftlab.dto.hr.EvalResultSimpleDto;
import com.example.clsoftlab.repository.hr.EvalDetailRepository;
...
public class EvalResultService {
	...
	private final EvalDetailRepository evaluationDetailRepository;
	...
	public Page<EvalResultDetailDto> findByPernr (String pernr, int page, int size) {
		...
		return evaluationResultRepository.findByEvalEmp_Pernr(pernr, pageable)
				.map(i -> modelMapper.map(i, EvalResultDetailDto.class));
	}
	
	// 사번으로 결과 simpleList 조회
	public List<EvalResultSimpleDto> getSimpleList (String pernr) {
		return evaluationResultRepository.findAllByTargetEmp_PernrAndEvalStepOrderByTemplate_EndDateDesc(pernr, "900").stream()
				.map(i -> modelMapper.map(i, EvalResultSimpleDto.class))
				.toList();
	}
...
//	// 평가 detail 저장
//	@Transactional
//	public void saveDetailList (List<EvalDetailRequestDto> dtoList) {
...
//		
//		for (EvalDetailRequestDto dto : dtoList) {
//			totalScore += dto.getPoint();
//			if (dto.getId() != null) { // id가 있는 항목은 수정
//				EvalDetail evaluationDetail = evaluationDetailRepository.findById(dto.getId())
//						.orElseThrow(() -> new EntityNotFoundException("해당 항목을 찾을 수 없습니다. id : " + dto.getId()));
//				evaluationDetail.update(dto);
//			}  else {
//				EvalDetail newDetail = modelMapper.map(dto, EvalDetail.class);
...
//	}
	...
	// resultId로 상세 점수  조회
	public List<EvalDetailDto> findById (Long id) {
		return evaluationDetailRepository.findByEvalResult_Id(id).stream()
				.map(i -> modelMapper.map(i, EvalDetailDto.class))
				.toList();
	}
}
